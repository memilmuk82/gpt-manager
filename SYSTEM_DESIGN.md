...
Landing/Auth/Dashboard/Reservations/Logs/Prompts/Profile/Admin Test Result: 문서화된 UI 시스템 적용
```
...
```text
앱 제목, 학교/부서명, 인증 안내, 업무게시판 안내, 로그아웃 안내, AI 활용 권장 순서, 기본 사용 시간, 최대 사용 시간을 관리자 화면에서 수정한다.
...
API Key 암호화
프롬프트 정리 mock
관리자 테스트 실행 결과의 파일별 검증 설명, 상태, 실패 원인 요약, 해결 힌트 표시
예약 자동 충돌 확인 및 최대 사용 시간 8시간(480분) 기본값 반영
