...
| [docs/adr/0002-use-gemini-for-prompt-review.md](docs/adr/0002-use-gemini-for-prompt-review.md) | 초기 Gemini 사용 범위를 볼 때 | 자유 채팅이 아닌 프롬프트 점검기로 제한한 이유 |
...
```text
이 프로젝트는 BYOK 방식입니다. 관리자는 공용 API Key를 제공하지 않습니다.
지원 Provider는 OpenAI, Google Gemini, Anthropic Claude 3개입니다. OpenRouter는 현재 지원하지 않습니다.
API Key 발급, 결제 설정, 사용량 관리는 각 외부 서비스 정책에 따르며 사용자 본인의 책임입니다.
모델명은 시간이 지나면 바뀔 수 있으므로 앱 내 모델 목록 새로고침 또는 공식 문서 확인이 필요합니다.
연결 테스트 호출도 소량의 비용이 발생할 수 있습니다.
학생 개인정보, 성적, 생활기록부 원문, 평가 자료, 상담 내용, 비공개 업무자료는 입력하지 않습니다.
API Key는 평문 저장하지 않으며 관리자 화면에서도 원문을 볼 수 없습니다.
Docker 운영에서 외부 공개 포트는 80/443 웹 포트만 허용하고 DB/Redis/내부 앱/개발 서버 포트는 공개하지 않습니다.
```
