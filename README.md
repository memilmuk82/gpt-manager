...
이 리뷰용 관리자 계정은 Google OAuth 계정이 아닙니다. 로그인 화면의 Google 로그인 버튼이 아니라 이메일과 비밀번호 입력 폼에 위 이메일과 초기 비밀번호를 직접 입력해야 로그인됩니다.
...
로컬 회원가입/로그인/로그아웃
Google OAuth 로그인
검증된 Google 계정 로그인 시 즉시 승인 및 관리자 권한 부여
도메인 제한 없는 계정 자동 승인
로컬 신규 계정 자동 승인
...
```text
1. Google 로그인 계정은 즉시 승인되며 관리자 권한으로 등록
2. 로컬 계정은 ADMIN_EMAILS 또는 ASSISTANT_ADMIN_EMAILS 설정에 따라 관리자 또는 보조관리자 권한 부여
3. /admin 에서 전체 요약 확인
