...
- Landing은 프로젝트명, 한 줄 설명, CTA, 핵심 기능 3-4개를 우선한다.
- 실제 화면 미리보기와 AI 비교 예시는 향후 추가 대상으로 둔다.
...
- Prompt 실행은 단계형 timeline으로 설계한다.
- 실행 중, 성공, 실패, 건너뜀을 단계별로 표시한다.

제한:

- 실제 multi-provider 실행은 현재 구현이 아니므로 문서에서 future pattern으로 분리한다.
