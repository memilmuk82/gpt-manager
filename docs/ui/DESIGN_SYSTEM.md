...
- AI가 작동하고 있다는 느낌을 주되 현재 단일 Provider 실행 범위를 과장하지 않는 입력, 진행 상태, 결과 검토 구조
...
- Landing은 첫 화면에서 프로젝트 이름, 한 줄 설명, CTA, 실제 구현 범위의 운영 미리보기를 보여준다.
...
- 프롬프트 실행은 입력, 준비, 선택 Provider 호출, 저장, 완료의 단일 Provider 단계로 표현한다.
- Timeline 색상은 실행 단계에만 제한적으로 사용하고 병렬 비교처럼 보이지 않게 한다.
...
- 결과 화면은 원본 요청, 단일 Provider 응답, 검토 메모, 위험 요소, 개선 제안을 분리한다.
...
| --- | --- | --- | --- |
| OpenAI / GPT | `#1f7a5f` | `#eef2f6` | `GPT` |
| Google / Gemini | `#3267b7` | `#edf9f7` | `Gemini` |
| Anthropic / Claude | `#8a5a16` | `#fff4e8` | `Claude` |
...
| --- | --- | --- | --- | --- |
| Hero Title | 30-36px | 900 | 1.22 | Landing 첫 화면 |
...
현재 CSS는 `--gm-radius-card: 12px`, `--gm-radius-button: 10px`, `--gm-radius-input: 10px`, `--gm-radius-modal: 16px`를 기준으로 한다. 반복 업무 화면은 8-12px 중심의 낮은 radius를 사용하고, badge처럼 상태 라벨인 경우에만 full radius를 허용한다.

| 컴포넌트 | Radius | Border | Shadow |
| --- | --- | --- | --- |
| Button | 8-12px | primary 1px same-color, secondary 1px | 약한 shadow 허용 |
...
- 현재 단계: Prompt 입력 -> 실행 준비 -> 선택 Provider 실행 -> 결과 저장 -> 완료.
- GPT/Gemini/Claude 병렬 실행과 비교 분석 단계는 백엔드 구현 전까지 UI에 표시하지 않는다.
...
2026-07-05 UI 적용 기준으로 실제 코드에 연결된 주요 클래스는 다음과 같다.
...
- dark canvas와 강한 gradient를 사용하지 않는다.
- 카드 안에 큰 카드를 중첩하지 않고 `gm-inset` 또는 행/목록 구조를 사용한다.
- table은 horizontal scroll을 허용하되 페이지 전체 수평 overflow는 만들지 않는다.
- form label은 visible text를 유지한다.
- 상태는 색상만이 아니라 `예약`, `완료`, `취소`, `활성`, `비활성`, `PASS`, `FAIL` 같은 텍스트를 포함한다.
