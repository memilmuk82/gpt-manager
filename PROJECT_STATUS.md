...
```text
상태: UI 디자인 시스템 실제 적용 및 문서 구조 최신화 완료
최근 검증일: 2026-07-05
...
월간 운영 보고서, 감사 로그, 최근 20개 DB 백업 보관, 조건별 CSV 내보내기, 전체 테스트 실행과 테스트 파일별 검증 설명 표시
/terms, /privacy 법적 고지 페이지와 공통 Footer 기관명/대표번호/링크
공통 UI 디자인 토큰, 낮은 radius, text badge, table overflow, visible label, focus-visible 적용
```
...
관리자 테스트 실행 화면: 테스트 파일별 검증 대상/주요 내용/상태 표시 확인
예약 UX: 자동 충돌 확인과 max_duration_minutes 화면 반영 확인
관리자 운영 도구: CSV 조건 필터와 백업 최근 20개 보관 정책 확인
UI 검증: Landing/Login/Dashboard/Reservations/Prompt 입력 desktop/mobile 수평 overflow 없음
```
