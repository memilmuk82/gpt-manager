...
월간 운영 보고서, 감사 로그, 최근 20개 DB 백업 보관, 조건별 CSV 내보내기, 전체 테스트 실행과 테스트 파일별 검증 설명/실패 원인/해결 힌트 표시
...
개인 Profile 화면의 계정/권한/활동/API Key 상태 요약
```
...
일반 사용자: 본인 프로필/예약/로그/API Key/프롬프트 정리, 오늘 예약 전체 현황 조회
...
```text
자유 채팅형 챗봇
multi-provider 비교 실행/streaming UI
수업안/평가계획 생성기
교육과정 파일 분석
학생 개인정보 처리
